...
<%
	...
	List myRequestList = null; // 내 판매완료글 리턴받을 변수
...
<%
	...
	if(sel != null && search != null) { // 검색일때 
		...
		if(count > 0) {
			// 검색한 글 목록 가져오기 
			myRequestList = dao.getGallerySearch(startRow, endRow, u_id, sel, search); 
			System.out.println("검색글 목록 : " + myRequestList);
		}
	}else if(startDate != null && endDate != null){ // 기간조회시,
		count = dao.getRequestDateCount(u_id, startDate, endDate);
		System.out.println("선택된 날짜 : " + startDate + " - " + endDate);
		System.out.println("해당기 등록건 수 : " + count);
		if(count > 0){
			myRequestList = dao.getRequestDate(startRow, endRow, u_id, startDate, endDate);
			System.out.println("해당기간 등록상품 : " + myRequestList);	
		}			
	}else { // 일반 게시판일때 
		// #1. 판매승인대기건 카운팅 메서드
		count = dao.getMyRequestCount(u_id);	
		System.out.println("판매승인 대기 상품 수 : " + count);
		// 글이 하나라도 있으면, 현재 페이지에 띄워줄 만큼만 가져오기 (페이지 번호 고려) 
		if(count > 0){
			// #2. 판매승인대기건 가져오는 메서드
			myRequestList = dao.getMyRequest(u_id, startRow, endRow);
			System.out.println("판매승인 대기 상품목록 : " + myRequestList);
		}
	}
...
<%         } %>
	...
	<%} %>
		...
		<form action="sellRequest.jsp" method="post" >
	...
	<%	}else{	// 작은else%>
			...
			<tr align="center">
			 ...
			 <% for(int i = 0; i < myRequestList.size(); i++){ %>
		  		<% GalleryDTO gallery = (GalleryDTO)myRequestList.get(i);%>
				...
				<tr>
					...
					<td>
						<a href="boardManageSelf.jsp?g_bno=<%=gallery.getG_bno()%>"><img src="/team/save/<%=gallery.getG_img()%>" width="100"></a>
					</td>
				...
				<% System.out.println("체크박스번호 : " + checkNum); %>
		  <% } %>
		</table>
	<%	} //if else의 끝 %>
	</div> <%-- main div의 끝 --%>
	 ...
	 <% if(count > 0) { 
			...
			if(startPage > pageNumSize) { 
				if(sel != null && search != null) { %>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=startPage-1%>&sel=<%=sel%>&search=<%=search%>"> &lt; &nbsp; </a>
				<%}else{%>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=startPage-1%>"> &lt; &nbsp; </a>
				<%}
			}
			
			for(int i = startPage; i <= endPage; i++) { 
				if(sel != null && search != null) { %>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=i%>&sel=<%=sel%>&search=<%=search%>"> &nbsp; <%= i %> &nbsp; </a>
				<%}else{ %>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=i%>"> &nbsp; <%= i %> &nbsp; </a> 
				<%} 
			}
			
			if(endPage < pageCount) { 
				if(sel != null && search != null) { %>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=startPage+pageNumSize%>&sel=<%=sel%>&search=<%=search%>"> &nbsp; &gt; </a>
			<%	}else{ %>
					<a class="pageNums" href="sellRequest.jsp?pageNum=<%=startPage+pageNumSize%>"> &nbsp; &gt; </a>
			<% } %>
		<%	}  %> 
	...
	<%	}//if count > 0 %>
		...
		<form action="sellRequest.jsp">
			...
			<input type="button" value="전체글로 돌아가기" onclick="window.location='sellRequest.jsp'"/>
		</form>
