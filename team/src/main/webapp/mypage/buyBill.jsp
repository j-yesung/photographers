...
<%
	...
	// img 이름 하나 가져오기
	//String imgName = dao.getImgName(g_bno); // 이미지 이름
	//String url = request.getRealPath("save");	//url경로
	//System.out.println("파일 저장경로 : " + url);
	//dao.fileDown(url, imgName);
...
<%
	...
	String p_id = (String)session.getAttribute("userId"); // 구매자 이름
%>	
</head>
<%
...
<%         } %>
	...
	<%} %>
		...
		<h2 align="left"> 상세구매내역 : 결제번호 <%=pno%> </h2>
		<table>
			<tr align="center">
				<td> No. </td>
				<td> 상품고유번호 </td>
		...
		<table>
			<tr>
				<td> 판매자 : <%=u_id%> / 상품금액 : <%=g_price%> / 사용포인트 : <%=p_point%> / 총결제금액 : <%=p_finalPrice%> / 결제상태 : <%="결제완료"%>
	...
	<%	}//if count > 0 %>
		...
		<form action="sellStatus.jsp">
			<input type="button" value="전체 구매글로 돌아가기" onclick="window.location='buyList.jsp'"/>
		</form>
