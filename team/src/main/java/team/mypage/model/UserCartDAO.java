...
public class UserCartDAO {
	...
	// 장바구니 추가
	public void insertCart(String u_id, String u_id2, int g_bno){
		Connection conn = null; 
		PreparedStatement pstmt = null; 
		try {
			conn = getConnection(); 
			String sql = "insert into userCart values(userCart_seq.nextval,?,?,?,sysdate)";
	...
	// 장바구니 삭제
	public void cartDelete(int ucno){
		int amount=0;
		Connection conn = null; 
		PreparedStatement pstmt = null; 
		ResultSet rs = null;
	...
	// 장바구니 검색 카운팅 메서드
	public int getCartSearchCount(String u_id, String sel, String search) {
		int count = 0; 
		Connection conn = null; 
		PreparedStatement pstmt = null; 
		ResultSet rs = null;
		...
		try {
			conn = getConnection(); 
			String sql = "select count(*) from (select g.*, c.uc_reg, c.ucno from galleryUpload g, userCart c "
				...
				if(rs.next()) {
					count = rs.getInt(1);
					System.out.println("cart Seach count : " + count);
				}
	...
	// 장바구니 검색 목록 가져오는 메서드
	public List getCartSearch(int start, int end, String u_id, String sel, String search) {
		List list = null; 
		Connection conn = null; 
		PreparedStatement pstmt = null; 
		ResultSet rs = null;
		...
		try {
			...
			String sql = "select * from (select rownum r, A.* from ("
					+ "select g.*, c.uc_reg, c.ucno from galleryUpload g, userCart c "
					+ "where g.g_bno = c.g_bno and c.u_id=? and g." +sel+" like '%"+search+"%'"
	...
	//장바구니 갯수 카운팅 메서드
	public int getMyCartCount(String u_id) {
		int count = 0;
		Connection conn = null; 
		PreparedStatement pstmt = null; 
		ResultSet rs = null;
	...
	//장바구니 목록 가져오는 메서드
	public List getMyCarts(String u_id, int startRow, int endRow) {
		List list = null; 
		Connection conn = null; 	
		PreparedStatement pstmt = null; 
		ResultSet rs = null;
		try {
			...
			String sql = "select * from (select rownum r, A.* from ("
					+ "select g.*, c.uc_reg, c.ucno from galleryUpload g, userCart c "
					+ "where g.g_bno = c.g_bno and c.u_id=? order by c.uc_reg desc) A ) "
					+ "where r >= ? and r <= ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, u_id);
	...
	// 장바구니 중복 확인 메서드
	public boolean confirmCart(int g_bno, String u_id) {
		...
		try {
			conn = getConnection(); 
			String sql = "select count(*) from userCart where g_bno=? and u_id=?";
			pstmt = conn.prepareStatement(sql);
